...
golang-migrate ledger is isolated in `public.vidra_search_migrations`, so it never
collides with core's `schema_migrations` — the table and schema names are
**compiled into the search binary**, `internal/dbmigrate`, not passed as the
`x-migrations-table` URL param the retired CLI container needed), and it uses
**Redis DB index 1** (core uses DB 0). Search
