...
# THE FIVE TLS MODES, validated once and named in one place. Every check below
# that behaves differently per topology reads $TLS_MODE, and none of them
# re-litigates which spellings exist:
#
#   acme | acme-staging   Vidra's Caddy terminates TLS, certificate from Let's
#                         Encrypt. Everything is checked: the Caddyfile, its
#                         domain, DNS, the reload and an https edge probe.
#   internal              Vidra's Caddy terminates TLS from its own CA. As above
#                         minus the DNS preflight (no ACME order is ever made).
#   external              SOMEBODY ELSE terminates TLS — an nginx/HAProxy in
#                         front, a cloud LB, a CDN. deploy/lib.sh withholds the
#                         `edge` profile, so this project has no caddy service at
#                         all: no Caddyfile to check, nothing to reload, and the
#                         domain deliberately resolves to that proxy and not to
#                         this host.
#   plain-http            Deliberate no-TLS (lab, LAN, air-gap). Caddy still
#                         runs, as a plain-HTTP site, so all of its machinery
#                         stays — but the origin may be an IP or an internal
#                         name, so the real-domain and DNS checks do not apply
#                         and the edge is probed over http.
#
# A value outside that list is REFUSED rather than defaulted: it decides whether
# this host runs a TLS edge at all, and guessing means either two edges fighting
# over :443 or none at all.
require_known_tls_mode() {
  case "$TLS_MODE" in
    acme|acme-staging|internal|external|plain-http) return 0 ;;
  esac
  die "VIDRA_TLS_MODE=$TLS_MODE is not one of: acme, acme-staging, internal, external, plain-http (see the TLS block in env/production.env.example, or re-run 'vidra setup'). This value decides whether this host terminates TLS, so it is not guessed at."
}
...
#
# Only for the ACME modes. The other three never make an ACME order from this
# host — `internal` issues from Caddy's own CA, `external` has no Caddy at all,
# and `plain-http` has no TLS to certify — so DNS is irrelevant to all of them,
# and for two of them the record legitimately does NOT point here.
#
...
should_skip_dns_preflight() {
  ...
  case "$mode" in
    ...
    internal)
      ...
      ;;
    external)
      log "VIDRA_TLS_MODE=external — TLS terminates in your proxy, not here, so no ACME order is ever made from this host and the domain is EXPECTED to resolve to that proxy rather than to this box. Skipping the DNS preflight."
      return 0
      ;;
    plain-http)
      log "VIDRA_TLS_MODE=plain-http — no TLS, no ACME, and the origin may be an IP or an internal name. Skipping the DNS preflight."
      return 0
      ;;
    *)
      die "VIDRA_TLS_MODE=$mode is not one of: acme, acme-staging, internal, external, plain-http (see env/production.env.example)"
      ;;
  esac
...
# Read ONCE, into a global every step below branches on — and with the same
# default (`acme`) deploy/lib.sh's edge_profile() uses, so "does this project
# contain a caddy service" and "does this script check the Caddyfile" can never
# answer differently.
TLS_MODE="$(env_get VIDRA_TLS_MODE acme)"
...
#
# Which of the three run at all is the mode's business, and every skip is
# PRINTED. A silently skipped safety check is worse than one that never existed:
# the operator keeps believing it ran.
case "$TLS_MODE" in
  external)
    log "VIDRA_TLS_MODE=external — your proxy/LB/CDN terminates TLS, so this project has NO caddy service (deploy/lib.sh withholds the 'edge' profile). SKIPPING: the deploy/Caddyfile.local check, its domain check and the DNS preflight. The edge configuration, its certificate and its routing are yours; deploy/nginx-external.conf.example is the reference server block."
    ;;
  plain-http)
    require_caddyfile_local
    require_real_domain
...
# artifact. A MIXED triple no record pairs is refused, fetch or no fetch.
# VIDRA_RELEASE_MAPPING=warn (env file or environment, like
...
if [ "$TLS_MODE" = "external" ]; then
  # Nothing to reload: there is no caddy service in this project at all. The
  # operator's proxy is a separate piece of software with its own reload — and
  # `compose exec caddy` here would fail with "no such service", which reads as a
  # broken deploy rather than as the intended topology.
  log "VIDRA_TLS_MODE=external — no caddy service in this project, so there is nothing to reload here. If you changed routing or upstreams, reload YOUR proxy (nginx -s reload / systemctl reload nginx / the LB's console)."
elif ! reload_caddy; then
  "${COMPOSE[@]}" logs --tail=50 caddy >&2 || true
  die "caddy reload did not succeed after 5 attempts, so the edge is NOT serving deploy/Caddyfile.local — either it never started, or it is still serving the PREVIOUS configuration. The application containers are up and the database has already been migrated; nothing was rolled back. Caddy's last 50 log lines are above. Reproduce the adapt failure by hand:
...
#
# -k because the certificate is NOT what is under test and every VIDRA_TLS_MODE
# that runs an edge has to pass here: `internal` issues from Caddy's own CA and
# `acme-staging` from a deliberately untrusted root, neither of which curl has
# any reason to trust. A genuinely broken chain still fails loudly in a browser;
# this asks only "does the edge answer for this domain".
#
...
#
# THE SCHEME AND THE DEADLINE ARE ARGUMENTS because the mode decides both:
# plain-http has no :443 to connect to, and `external` has nothing this deploy
# started, so there is nothing to WAIT for — it is one attempt or none.
EDGE_HOST="$(url_host "$(env_get PUBLIC_BASE_URL '')")"
probe_edge() {
  local scheme="$1" host="$2" timeout="$3" port deadline
  case "$scheme" in
    https) port=443 ;;
    *)     port=80  ;;
  esac
  deadline=$(( $(date +%s) + timeout ))
  log "waiting up to ${timeout}s for the edge at ${scheme}://${host}/healthz (pinned to 127.0.0.1:${port})"
  until curl -fsSk -m 5 -o /dev/null --resolve "${host}:${port}:127.0.0.1" "${scheme}://${host}/healthz"; do
    if [ "$(date +%s)" -ge "$deadline" ]; then
      return 1
    fi
    sleep 3
...
if [ -z "$EDGE_HOST" ]; then
  ...
  # it may be an nginx on this very host (in which case this probe is worth
  # having and usually passes), or a load balancer two networks away that
  # 127.0.0.1 can never reach. Failing the deploy on the second case would report
  # a successful deploy as broken. Nothing here was started by this run either,
  # so there is nothing to wait for — hence the zero deadline.
  if ! probe_edge https "$EDGE_HOST" 0; then
    printf '[deploy] WARNING: nothing answered https://%s/healthz on this host'"'"'s :443, so the edge was NOT verified. With VIDRA_TLS_MODE=external that is expected when your proxy/LB/CDN runs somewhere else — the deploy itself succeeded, and the api and frontend are healthy on 127.0.0.1:%s and :%s. If the proxy IS on this host, it is not routing yet: check its config against deploy/nginx-external.conf.example (upstreams, X-Forwarded-*, the /api /healthz /readyz /version /feeds /nodeinfo /.well-known split), then run '"'"'vidra doctor'"'"'.\n' \
      "$EDGE_HOST" "$HTTP_PORT" "$FRONTEND_PORT" >&2
  fi
elif [ "$TLS_MODE" = "plain-http" ]; then
  # Same probe, no TLS: Caddy is serving an `http://` site, so :443 is not
  # listening at all and https would fail for the right reason and the wrong one
  # at once.
  if ! probe_edge http "$EDGE_HOST" "$EDGE_TIMEOUT"; then
    printf '[deploy] ERROR: the plain-HTTP edge never answered for %s. The api and frontend are healthy on the loopback, so this is Caddy: a Caddyfile.local that adapts but routes wrong, or a container that is not running. Recent caddy logs:\n' "$EDGE_HOST" >&2
    "${COMPOSE[@]}" logs --tail=50 caddy >&2 || true
    printf '[deploy] Check by hand:\n  ./deploy/compose.sh ps caddy\n  ./deploy/compose.sh exec caddy caddy validate --config /etc/caddy/Caddyfile\n  ./deploy/compose.sh logs --tail=100 caddy\n' >&2
    rc=1
  fi
else
  if ! probe_edge https "$EDGE_HOST" "$EDGE_TIMEOUT"; then
