...
def paths(argv):
 """argv[1] is the runtime stage (default: the v0.6.4 run); argv[2] the optional
 B2 completion stage. Every other path derives from the stage's name, so a
 v0.6.5 run at /root/vidra-v065-runtime exports to
 /root/vidra-v065-reviewed-evidence{,.tar.gz} and reads its bucket key from
 /root/vidra-v065-b2-key.json."""
 stage=Path(argv[1]) if len(argv)>1 else Path('/root/vidra-v064-runtime')
 assert stage.name.endswith('-runtime'),'stage directory must be named <label>-runtime'
 completion_stage=Path(argv[2]) if len(argv)>2 else None
 label=stage.name[:-len('-runtime')]
 return (stage,completion_stage,stage.parent/f'{label}-reviewed-evidence',
         stage.parent/f'{label}-reviewed-evidence.tar.gz',stage.parent/f'{label}-b2-key.json')

def main(argv=None):
 stage,completion_stage,out,archive,key_path=paths(sys.argv if argv is None else argv)
