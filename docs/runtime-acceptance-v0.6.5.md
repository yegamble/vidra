...
The harness was made release-agnostic for this run (meta#200, merged
2026-09-13 15:59 UTC after a review round: `prepare --candidate
docs/evidence/release-v0.6.5-verification/manifest.json`; the handoff records
the candidate tag and `run` refuses a handoff prepared for another release).
...
sha256 of that file on `main`. A fresh `deploy/release-preflight.py --tag
v0.6.5 --platform linux/amd64` run was made only to re-confirm status PASS and
to supply the frozen sources and assets `prepare` compares against the
manifest. `handoff.json` sha256 `579ae192…2314d` (as `result.json` records it)
and the tarball sha256 `5a51ef3f…8db195` are session-reported; neither file is
committed.

Host: droplet **599531580** (159.203.118.182), rebuilt blank by the operator at
**15:29:26 UTC**, native Ubuntu **24.04.4 LTS / x86_64**, 8 vCPUs; the
hostname `vidra-beta-v064-b2-acceptance` is retained across rebuilds.
`result.json`'s `before.existing` is empty (no `/opt/vidra`, no
`/usr/local/bin/vidra`, no `/var/lib/docker`); `check-host` passed (root,
systemd, no container). The droplet id, rebuild timestamp and vCPU count are
operator-reported, not reconstructible from the committed artifacts. The browser used `https://secure.video.test` inside the guest.
...
| Real transcode | Durable job `fa6dbfdd-8426-45ca-a454-e9c61cd8fe7d` observed `running` → **`done`**, retry counter **0**; a 360p CMAF rendition and **12** advertised playlist/init/fragment assets fetched successfully; 28 media requests recorded (thumbnail 200, original 206, segments 200, one view beacon 204) |
...
(recomputed over every committed file, including `export-hashes.json` and the
disposition).
...
Exporter secret scan PASS and archive sha256
`efa55c0c942c54ee90a13f03e0c3a766037d842aa4e1edd5b21d42079a30825e` are
session-reported (the archive is not committed). `export-provenance.json`'s
raw hashes are asserted by the exporter, not reconstructible here; of the two
exported JSON files only `result.json` was transformed (health-log outputs
replaced by their hashes) — `browser-result.json` is byte-identical to its
raw (`raw_browser_sha256`).
...
(session-reported; `host-commands.json` records the run itself as one
contiguous sequence, 001–096, on a single evidence directory) and the
harness's own reuse guards were satisfied.
