...
- Presigned-URL / CDN offload for public HLS bytes (`storage.go` capability interface + a `redirectPresigned` helper modelled on the existing `ipfs_assets.go:15` 307 pattern). This is a cost ceiling, not a defect — every viewed byte is billed twice today (Spaces→droplet + droplet→viewer).
